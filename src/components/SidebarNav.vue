...
<template>
  ...
  >
    <!-- Search -->
    <div class="p-4 pb-2">
      <div class="border-default-200 bg-default-100/30 flex items-center gap-2 rounded-kun-md border px-3 py-2 text-sm transition-colors focus-within:border-primary">
    ...
    <!-- Nav groups (forum-style) -->
    <div class="flex-1 overflow-y-auto px-3 space-y-5 scrollbar-hide">
      <!-- Group: 导航 -->
      <div>
        <p class="text-default-400 mb-1.5 px-2 text-xs font-semibold tracking-wide uppercase">导航</p>
        <div class="flex flex-col gap-0.5">
          <KunButton
            variant="light"
            color="default"
            size="sm"
            :class="!route.query.f ? '!text-primary !bg-primary/10 font-medium' : ''"
      ...
      <!-- Group: 贴文列表 -->
      <div>
        <div class="flex items-center justify-between px-2 mb-1.5 relative">
          <p class="text-default-400 text-xs font-semibold tracking-wide uppercase">
          ...
          <button class="text-default-400 hover:text-foreground cursor-pointer p-1 rounded-kun-sm hover:bg-default-100 transition-colors text-xs" @click.stop="toggleSortMenu">
            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
              <path d="M11 5h10M11 12h7M11 19h4"/><path d="M4 5l3 3-3 3"/>
            </svg>
          </button>
          <div v-if="showSortMenu" class="z-kun-popover absolute right-0 top-6 min-w-[140px] bg-default-100 border border-default-200 rounded-kun-md shadow-lg overflow-hidden" @click.stop>
            <button class="w-full flex items-center justify-between px-3 py-2 text-xs text-foreground hover:bg-default-200 transition-colors" :class="{ '!text-primary font-medium': currentSortMode === 'date' }" @click="applySort('date')">
              <span>📅 按时间</span>
            ...
            <button class="w-full flex items-center justify-between px-3 py-2 text-xs text-foreground hover:bg-default-200 transition-colors" :class="{ '!text-primary font-medium': currentSortMode === 'title' }" @click="applySort('title')">
              <span>Aa 按标题</span>
        ...
        <!-- File tree / loading / search -->
        <div class="overflow-y-auto max-h-[calc(100vh-380px)]">
          <!-- Loading -->
          <div v-if="loading" class="flex flex-col items-center gap-2 py-6 text-default-400 text-xs">
          ...
          <!-- Error -->
          <div v-else-if="error" class="px-3 py-3 text-danger text-xs">❌ {{ error }}</div>
    ...
    <!-- Footer -->
    <div class="border-default-200 mt-auto flex items-center justify-between border-t px-5 py-3 text-default-400 text-xs">
